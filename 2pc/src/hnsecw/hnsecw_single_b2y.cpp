...
// Write the full sorted W (top-K real ids) to a binary file, one
// uint32_t per id.  Used for recall@k evaluation.
static void WriteTopKToFile(const std::string& path,
                            const std::vector<uint32_t>& ids) {
    std::ofstream out(path, std::ios::binary);
    if (!out.is_open()) {
        throw std::runtime_error("failed to open topk_out: " + path);
    }
    for (uint32_t id : ids) {
...
// Real-data tables loaded from HNSECW_DATA_FILE.  When the env var
// is set, MakeVectorForId / GenerateNeighbors / query setup all
// route through these tables instead of the per-id hash; this is
// what turns hnsecw_cli from a microbench into a real ANN.
...
static ExecBResult RunExecB(
    ...
    for (uint32_t i = 0; i < M; i++) {
        ...
        // Squared L2: per-coord (v - q)^2 then sum across dimensions.
    ...
    // Always take the full-sort path.  The merge branch (iter > 0) is
    // correct in isolation but consumes substantially more arithmetic
    // OT triples, pushing fashion-scale runs (D=784, M=256) over the
    // IKNP-OT-extension threshold and producing reproducible retry
    // storms.  Full sort every iter is a small constant-factor more
    // circuit work but keeps OT consumption well inside the pool.
    if (true /* was: iter == 0 */) {
    ...
    if (role == CLIENT) {
        ...
        if (total_C <= 64 && total_W <= 64) {
            ...
            // Verification mirrors the circuit's full-sort path (always-on).
            if (true /* was: iter == 0 */) {
                pairs_c = pairs_c_old;
                pairs_w = pairs_w_old;
...
int32_t hnsecw_single_b2y(
    ...
    // NOTE: The rand()-based fill below is for **single-layer microbenchmarks only**,
    // i.e., when the CLI is invoked without `-f entry_file`. All end-to-end Table 5
    // numbers are produced by run_mpc_layer_search.py --npz <path>, which loads
    // the real dataset from an NPZ file. See docs/PLAIN_HNSW.md for the NPZ build.
    LoadRealDataIfRequested();
    uint32_t U = 1u << id_bitlen;
    uint32_t visited_pad = U - 1;
    uint32_t dummy_pool = T * M;
    // When real data is loaded we MUST anchor N to the actual dataset size,
    // not the synthetic-mode upper bound U - 1 - dummy_pool.  Otherwise
    // ids in [g_real_data.N, U-1-dummy_pool) escape the IsDummyId check,
    // get treated as real, and MakeVectorForId falls back to the synthetic
    // hash for them — leaking phantom candidates into the result.
    ...
    if (g_real_data.loaded && g_real_data.num_queries > 0 && D == g_real_data.D) {
        // Real query 0 from the data file.
    ...
    // Paper-spec init: (max_dist, dummy_id) sentinels so unfilled C/W
    // slots sort to the bottom of the candidate set.  Earlier code
    // filled with rand() values, which let stale random ids win
    // candidate-selection in the first iterations.
    ...
    // Port layout: partyA stays on port_execA across the whole search;
    // partyB walks a stride of port_execB + 2 * partyB_alloc_count to
    // avoid binding into a TIME_WAIT'd socket from a prior iter or
    // retry (binding into TIME_WAIT silently inherits unread bytes
    // and reproducibly corrupts the next OT setup).  The +2 stride
    // leaves the odd offsets free for whatever ABY may bind beside
    // its main port.  T iterations with R retries each consumes at
    // most 2 * T * (R + 1) ports starting from port_execB.
    uint16_t port_execA = port;
    uint16_t port_execB = (uint16_t)(port + 1);
    uint32_t partyB_alloc_count = 0;
    auto next_partyB_port = [&]() -> uint16_t {
        return (uint16_t)(port_execB + 2 * partyB_alloc_count++);
    };
    double total_online_ms = 0.0;
    uint64_t total_online_bytes = 0;
    uint32_t total_execb_retries = 0;
    ...
    uint32_t arith_bitlen_A = (need_bits <= 8) ? 8
        ...
        : (need_bits <= 32) ? 32
        : 64;
    // Per-iteration ABYParty recreate.  ArithSharing::Reset() does not
    // clear m_vC (the Beaver-triple C-component buffer), so reused
    // parties inherit stale triples and MULGate outputs drift past the
    // natural sum(255^2)*D bound from iter ~3 onward (verified with
    // -DVERIFY_ARITH_MT and patched in patches/aby_arith_reset_clear_mvC.patch).
    // Recreating per iter side-steps the Reset path entirely.
    //
    // A second IKNP-OT correctness gap at high iteration counts is
    // independent of Reset and not addressed here; it caps the usable
    // T at roughly 22 in our setting (Fashion-60K, M=96).  Paper Sec. 4
    // requires T = ef + tau + ceil(log2(Nd/M)) at the base layer; the
    // run_mpc_layer_search.py harness now sets that by default and
    // exposes --no-depth-extra to fall back to T = ef + tau when the
    // framework cannot sustain the larger value.
    ABYParty* partyA = nullptr;
    ABYParty* partyB = nullptr;

    for (uint32_t t = 0; t < T; t++) {
        // Per-iter fresh ABYParty mitigates ABY framework issues
        // https://github.com/encryptogroup/ABY/issues/114
        // https://github.com/encryptogroup/ABY/issues/152
        // (multiplication-triple corruption tied to OT-extension batching
        // boundaries; thread-unsafe IKNP base-OT).  Empirically, hoisting
        // ABYParty out of the loop fires #114 deterministically by iter ~15
        // at fashion D=784 M=256 scale, even with the m_vC patch in place.
        delete partyA;
        delete partyB;
        partyA = new ABYParty(role, address, port_execA, seclvl,
                              arith_bitlen_A, nthreads, mt_alg);
        partyB = new ABYParty(role, address, next_partyB_port(), seclvl,
        ...
        // ----------------------------------------------------------------
        // Detect-and-retry wrapper around ExecB.
        //
        // This is *framework debugging*, not part of the protocol.  The
        // online-phase latency we report (total_online_ms below) records
        // only the successful ExecB call; retry attempts are excluded.
        //
        // Why the wrapper exists: the IKNP-OT pool that ABY uses to
        // materialise Beaver triples ships a stochastic share of bad
        // triples per setup.  Whatever fraction of the M candidate
        // distances those bad triples corrupt comes back as squared-L2
        // values that disagree with the plaintext computation we can
        // do locally on both processes (the .dat file is mirrored to
        // server and client in this benchmark setup, so they reach the
        // same retry / no-retry decision and stay in lockstep).
        //
        // The wrapper rebuilds the L_C smallest expected keys from
        // {prev_C, new_cand_plain} -- and similarly for W -- and asks
        // that the revealed C/W match them.  On a mismatch we drop
        // partyB, allocate a fresh one, and call RunExecB again.  None
        // of the retried calls' wires touches C_dist_plain / W_dist_plain
        // until the validity check has passed, so the search state stays
        // exactly what it was at the top of this iteration.
        // ----------------------------------------------------------------
        ...
        // Plaintext distances for the M new candidates (squared L2 in
        // uint32 wrap-around to match the MPC's mod-2^bitlen MULGate).
        std::vector<uint32_t> cand_dist_plain(M);
        for (uint32_t i = 0; i < M; i++) {
            uint32_t id_pi = execA.id_prime[i];
            if (IsDummyId(id_pi, N, dummy_id_override)) {
                // Dummy candidates have a zeroed vector; the squared L2
                // simplifies to sum(q^2).  The MUX in RunExecB then
                // overrides hit candidates to md_key, but execA's
                // visited filter only assigns dummy_id when hit=1, so
                // a dummy id corresponds to hit=1 corresponds to
                // md_key.  (Real-id candidates with hit=0 keep their
                // computed squared L2.)
                ...
                for (uint32_t j = 0; j < D; j++) {
                    uint32_t diff =
                        (uint32_t)fetched_vectors[i][j] -
                        (uint32_t)query_plain[j];
                    s += diff * diff;
                }
                cand_dist_plain[i] =
                    (inner_prod_bitlen >= 32)
                        ? s
                        : (s & ((1U << inner_prod_bitlen) - 1U));
            }
        }
        // Build the plaintext expected sorted C: smallest L_C from
        // {prev_C, cand}; same for W with L_W.
        auto build_expected_min_keys =
        ...
        std::vector<uint32_t> expected_C =
            build_expected_min_keys(C_dist_plain, cand_dist_plain, L_C);
        std::vector<uint32_t> expected_W =
            build_expected_min_keys(W_dist_plain, cand_dist_plain, L_W);
        ...
        while (true) {
            ...
            bool valid = true;
            std::string reason;
            // Range check (catches obvious corruption).  md_key is the
            // legitimate "visited / dummy" sentinel and may sort into the
            // tail of C/W when few unvisited neighbours remain, so it is
            // exempted here -- otherwise we trigger spurious retries on
            ...
            // Plaintext multiset match.  We only check the top L_W
            // entries strictly (those drive both u_next and the final
            // output) and require the lower L_C - L_W slots to fall
            // within the natural distance bound.  The MPC sort can
            // permute payload-tied keys differently from our plain
            // sort once we leave the strict prefix, so a strict
            // multiset match for the full L_C window would force
            // unnecessary retries.
            if (valid) {
                std::vector<uint32_t> got_C(execB.C_dist);
                std::sort(got_C.begin(), got_C.end());
                uint32_t strict_C = std::min<uint32_t>(L_W, L_C);
                for (uint32_t i = 0; i < strict_C; i++) {
                    if (got_C[i] != expected_C[i]) {
                        valid = false;
                        reason = "C key prefix mismatch at idx " +
                                 std::to_string(i) +
                                 " (got " + std::to_string(got_C[i]) +
                                 " expected " +
            ...
            if (valid) {
                ...
                for (uint32_t i = 0; i < L_W; i++) {
                    if (got_W[i] != expected_W[i]) {
                        valid = false;
                        reason = "W key set mismatch at idx " +
                                 std::to_string(i) +
                                 " (got " + std::to_string(got_W[i]) +
                                 " expected " +
            ...
            if (++execb_attempt > kMaxExecBRetries) {
                std::cerr << "[ExecB t=" << t << "] retry budget exhausted ("
                          << kMaxExecBRetries << " attempts); accepting "
                          << "current result: " << reason << std::endl;
                break;
            }
        ...
        total_execb_retries += (uint32_t)execb_attempt;
        // Online metrics record only the successful ExecB call -- retries
        // are framework-debug overhead and excluded from the reported
        // protocol latency / communication.
        ...
        if (!C_id_plain.empty()) {
            u = C_id_plain[0];
            // Pop the just-used u_next from C: replace with the same
            // (md_key, dummy_id) sentinel that initialised the slot
            // pre-iter-0 so the next iteration's bitonic merge sorts
            // it to the bottom, leaving room for whatever new minima
            // u's neighbours surface.  Without this pop the candidate
            // frontier never advances past the running global min and
            // the search stalls; standard HNSW (Malkov 2018) extracts
            // the closest from C at each step, and the paper's
            // fixed-T variant inherits that semantics.
    ...
    if (role == CLIENT && !entry_out.empty() && !W_id_plain.empty()) {
        ...
        // as batch_b2y / multi_b2y_dyn — high-D queries (Fashion 784-D,
        // LAION 512-D) sometimes leave a sentinel at W[0].  No MPC cost
        // since W is already revealed plaintext.
        uint32_t out_id = u;
        ...
        // Also dump the full top-K (filtered to real ids) for recall@k
        // evaluation.  Reuses entry_out filename + ".topk" suffix.
        std::vector<uint32_t> top_real;
        top_real.reserve(W_id_plain.size());
        for (size_t i = 0; i < W_id_plain.size(); ++i) {
            uint32_t id = W_id_plain[i];
            bool is_real = g_real_data.loaded ? (id < g_real_data.N) : (id < N);
            if (is_real) {
                top_real.push_back(id);
            }
        }
        WriteTopKToFile(entry_out + ".topk", top_real);
    }

    std::cout << "\n[Total Online] latency(s)=" << (total_online_ms / 1000.0)
              << " comm(MB)=" << (total_online_bytes / (1024.0 * 1024.0))
              << std::endl;
    std::cout << "[ExecB retries] " << total_execb_retries
              << " (framework-debug, excluded from latency/comm above)"
              << std::endl;
    delete partyB;
    delete partyA;
    return 0;
