...
auto cluster_data = RandData(429539, dims);
auto stash = RandIdData(50649, 1, total_points_num);
auto ps = RandData(total_points_num, dims);
auto ptoc = RandIdData(378890, fake_cluster_points, total_points_num);
...
int main(int argc, char** argv) {
  ...
  auto encoded_db = PirData(cluster_num, fake_size, ps, ptoc, pir_logt,
                            fake_cluster_points, pir_fixt);
  auto mpir_server = PrepareMpirServer(batch_size, cluster_num, fake_size, lctx,
  ...
  FixPirResult(pir_s, pir_logt, pir_fixt, logt,
               pir_s.size() * fake_cluster_points, dims + 2 * message_size,
               kctx);
  auto fix_pir_s = FixPirResultOpt(pir_s, pir_logt, pir_fixt, logt,
                                   pir_s.size() * fake_cluster_points,
                                   dims + 2 * message_size, dims, rank, kctx);

  // fix_pir_s = FixPirResultOpt(pir_s, pir_logt, pir_fixt, logt,
  // pir_s.size() * fake_cluster_points,
  // dims + 2 * message_size, dims, rank, kctx);
  ...
  GcEndTopk(random_dis, random_id, min_value, min_index, logt - pointer_dc_bits,
