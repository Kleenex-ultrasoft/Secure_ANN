...
struct MultiQueryOptions {
  ::spu::seal_pir::SealPirOptions seal_options;
...
class MultiQueryServer : public MultiQuery {
 public:
  MultiQueryServer(const MultiQueryOptions &options,
    ...
    ::spu::seal_pir::SealPirOptions pir_options{
        query_options_.seal_options.poly_modulus_degree, max_bin_item_size_,
        query_options_.seal_options.element_size};

    SPDLOG_INFO("N:{} Max bin size:{} Element size:{}",
                query_options_.seal_options.poly_modulus_degree,
                max_bin_item_size_, query_options_.seal_options.element_size);
  ...
  void SetPublicKeys(const seal::PublicKey &pubkey, size_t dim) {
    for (size_t idx = 0; idx < pir_server_.size(); ++idx) {
      pir_server_[idx]->SetPublicKey(pubkey, dim);
    }
  }
...
class MultiQueryClient : public MultiQuery {
 public:
  ...
  seal::GaloisKeys GenerateGaloisKeys(size_t index) {
    return pir_client_->GenerateGaloisKeys(index);
  }
  seal::PublicKey GetPublicKey(size_t dim) {
    return pir_client_->GetPublicKey(dim);
  }
  ...
  std::vector<std::vector<seal::Ciphertext>> GenerateMultiQuery(
